...
pub async fn run(config: &Config, db: &Database, gh: &GhClient, args: &FixArgs) -> Result<()> {
    ...
    let pr_body = format!(
        ...
        config.branding.comment_marker(),
    );

    let pr_title = format!("fix: {} (#{}) [{}]", issue.title, issue.number, config.branding.name);

    match gh.create_pr(&pr_title, &pr_body, &branch, "main").await {
        Ok(pr_number) => {
            println!("Opened PR #{pr_number}: {pr_title}");
            let comment = format!(
                "I've opened PR #{pr_number} with a potential fix for this issue.\n\n{}",
                config.branding.comment_marker(),
            );
